...
public:
    ...
    FormGb(const Hand &ready, const T37 &pick, const FormCtx &ctx, bool juezhang);
    FormGb(const Hand &full, const FormCtx &ctx, bool juezhang);
    ~FormGb() = default;
...
private:
    void init13(const FormCtx &ctx);

    static int calcFan(const Fans &fs);

    Fans calcFansF7(const FormCtx &ctx, const Hand &hand) const;
    Fans calcFansF4(const FormCtx &ctx, const Hand &hand, const T37 &last,
    ...
    void checkV8F4(Fans &res, const Explain4 &exp, const FormCtx &ctx) const;
    ...
    void checkV2F4(Fans &res, const Explain4 &exp, const FormCtx &ctx,
                   const Hand &hand, const T37 &pick) const;
    void checkV1F4(Fans &res, const Explain4 &exp, const FormCtx &ctx, const Hand &hand) const;

    void checkPick(Fans &fans, const FormCtx &ctx) const;
